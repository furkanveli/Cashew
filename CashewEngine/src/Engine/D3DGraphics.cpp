...
namespace Cashew
{
	...
	void D3DGraphics::SetupDevice()
	{
...
#endif
		// LogHardware();
		/*D3D_FEATURE_LEVEL featureLevels[1] = { D3D_FEATURE_LEVEL_12_2 };
		...
		auto x = featureLevelsInfo.MaxSupportedFeatureLevel;
		ENGINE_TRACE("{}", x);*/
		...
		m_device->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS,&msQualityLevels,sizeof(msQualityLevels)) >> chk;

		m_msQuality = msQualityLevels.NumQualityLevels;
		...
		const DXGI_SWAP_CHAIN_DESC1 swapChainDesc =
		{
			...
			.SampleDesc =
			{
				m_msEnabled ? (UINT)4 : (UINT)1,
				m_msEnabled ? (m_msQuality- 1) : (UINT)0,
			},
		...
		const CD3DX12_RESOURCE_DESC depthDesc = CD3DX12_RESOURCE_DESC::Tex2D(
			...
			m_width, m_height, 1, 0, m_msEnabled ? 4 : 1, m_msEnabled ? (m_msQuality - 1) : 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
	...
	void D3DGraphics::SetupGeometry()
	{
		...
		const auto eyePosition = XMVectorSet(0, 0, -4, 1);
		...
		const auto projection = XMMatrixPerspectiveFovLH(XMConvertToRadians(90.f), aspectRatio, 0.1f, 100.0f);
	...
	void D3DGraphics::LogHardware()
	{
		...
		for (size_t i = 0; i < adapterList.size(); ++i)
		{
			...
			while (adapterList[i]->EnumOutputs(y, &output) != DXGI_ERROR_NOT_FOUND)
			{
				...
				output->GetDisplayModeList(m_backBufferFormat, flags, &count, &modeList[0]);
	...
	void D3DGraphics::Init()
	{
		...
		CreateBuffer(m_colorBuffer, m_colorArray, BufferType::Vertex);
	...
	void D3DGraphics::OnResize()
	{
		if (m_device && m_swapChain && m_commandAllocator)
		{
			...
			m_swapChain->ResizeBuffers(m_bufferCount, m_width, m_height, m_backBufferFormat, DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH) >> chk;
			...
			depthStencilDesc.SampleDesc.Count = m_msEnabled? 4 : 1;
			depthStencilDesc.SampleDesc.Quality = m_msEnabled ? (m_msQuality - 1) : 0;
